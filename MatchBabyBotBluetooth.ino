...
void setup() {
  wireless.begin(9600); // Enable serial communications over the USB cable
...
void loop() {
  if(!matchOver) {
    while(wireless.available()) { // Check to see if there is a new character on the serial port (check if the driver has pressed a button). If there is...
      ...
      int input = wireless.read(); // assign the character to the variable input
